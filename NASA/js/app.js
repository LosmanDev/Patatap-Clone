...
getNasaData().then(result => {
  ...
  let copyrightNa = document.querySelector(".nasa_copyright");
  if ((result.media_type = "image")) {
    media.innerHTML = `<img class="nasa_img img-responsive img-fluid" src=${result.url} alt="" srcset="">`;
    link.innerHTML = `<a class="btn btn-outline-light font-weight-bold" href="${result.hdurl}" target="_blank">Link to Source</a>`;
  } else {
  ...
  copyrightNa.innerHTML = result.copyright;
  console.log(result);
});
    ...
    $.ajax({
      ...
      success: function(r) {
        ...
        if ((r.media_type = "image")) {
          ...
          link.innerHTML = `<a class="btn btn-outline-light font-weight-bold" href="${r.hdurl}" target="_blank">Link to Source</a>`;
        } else {
